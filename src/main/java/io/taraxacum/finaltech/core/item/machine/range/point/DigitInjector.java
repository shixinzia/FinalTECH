...
/**
 * Can change the setting of a slimefun item as it is marked as {@link DigitInjectableItem}.
 * The effect varies depending on the item.
 * @see DigitInjectableItem
 * @author Final_ROOT
 */
public class DigitInjector extends AbstractPointMachine implements RecipeItem {
    private final Set<String> notAllowedId = new HashSet<>(ConfigUtil.getItemStringList(this, "not-allowed-id"));

    public DigitInjector(@Nonnull ItemGroup itemGroup, @Nonnull SlimefunItemStack item) {
    ...
    protected void tick(@Nonnull Block block, @Nonnull SlimefunItem slimefunItem, @Nonnull LocationData locationData) {
        ...
        if (ItemStackUtil.isItemNull(itemStack) || !(SlimefunItem.getByItem(itemStack) instanceof DigitalItem digitalItem)) {
            return;
        }
        ...
        if (sfItem instanceof DigitInjectableItem digitInjectableItem) {
            Runnable runnable = () -> {
                digitInjectableItem.injectDigit(tempLocationData, digitalItem.getDigit());
    ...
    public void registerDefaultRecipes() {
        ...
        for (SlimefunItem slimefunItem : Slimefun.getRegistry().getEnabledSlimefunItems()) {
            if (slimefunItem instanceof DigitInjectableItem) {
