 ...
 * @author Final_ROOT
 */
public class EnergizedStorage extends AbstractStorageMachine implements RecipeItem {
    private final Map<Location, Map<String, ItemWithAmount>> locationMap;
    private final String amountLimit = ConfigUtil.getOrDefaultItemSetting("INFINITY", this, "limit-amount");
    private final int typeLimit = ConfigUtil.getOrDefaultItemSetting(32, this, "limit-type");

    public EnergizedStorage(@Nonnull ItemGroup itemGroup, @Nonnull SlimefunItemStack item) {
        ...
        return new BlockBreakHandler(false, false) {
            public void onPlayerBreak(@Nonnull BlockBreakEvent blockBreakEvent, @Nonnull ItemStack itemStack, @Nonnull List<ItemStack> drops) {
                ...
                if(FinalTech.getLocationDataService() instanceof SlimefunLocationDataService slimefunLocationDataService) {
                    ...
                    if(blockMenu != null && blockMenu.getPreset().getID().equals(EnergizedStorage.this.getId())) {
                        Inventory inventory = blockMenu.toInventory();
                        InventoryUtil.dropItems(inventory, location, EnergizedStorage.this.getInputSlot());
                        InventoryUtil.dropItems(inventory, location, EnergizedStorage.this.getOutputSlot());
                    }
                }

                EnergizedStorage.this.locationMap.remove(location);
            }
        };
    }
