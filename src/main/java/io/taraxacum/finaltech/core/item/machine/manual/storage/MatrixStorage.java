 ...
 * @author Final_ROOT
 */
public class MatrixStorage extends AbstractStorageMachine implements RecipeItem {
    ...
    public MatrixStorage(@Nonnull ItemGroup itemGroup, @Nonnull SlimefunItemStack item) {
        ...
        return new BlockBreakHandler(false, false) {
            public void onPlayerBreak(@Nonnull BlockBreakEvent blockBreakEvent, @Nonnull ItemStack itemStack, @Nonnull List<ItemStack> drops) {
                ...
                if(FinalTech.getLocationDataService() instanceof SlimefunLocationDataService slimefunLocationDataService) {
                    ...
                    if(blockMenu != null && blockMenu.getPreset().getID().equals(MatrixStorage.this.getId())) {
                        Inventory inventory = blockMenu.toInventory();
                        InventoryUtil.dropItems(inventory, location, MatrixStorage.this.getInputSlot());
                        InventoryUtil.dropItems(inventory, location, MatrixStorage.this.getOutputSlot());
                    }
                }

                MatrixStorage.this.locationMap.remove(location);
            }
        };
    }
