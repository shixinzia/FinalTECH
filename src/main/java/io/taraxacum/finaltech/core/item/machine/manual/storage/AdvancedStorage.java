 ...
 * @author Final_ROOT
 */
public class AdvancedStorage extends AbstractStorageMachine implements RecipeItem {
    ...
    public AdvancedStorage(@Nonnull ItemGroup itemGroup, @Nonnull SlimefunItemStack item) {
        ...
        return new BlockBreakHandler(false, false) {
            public void onPlayerBreak(@Nonnull BlockBreakEvent blockBreakEvent, @Nonnull ItemStack itemStack, @Nonnull List<ItemStack> drops) {
                ...
                if(FinalTech.getLocationDataService() instanceof SlimefunLocationDataService slimefunLocationDataService) {
                    ...
                    if(blockMenu != null && blockMenu.getPreset().getID().equals(AdvancedStorage.this.getId())) {
                        Inventory inventory = blockMenu.toInventory();
                        InventoryUtil.dropItems(inventory, location, AdvancedStorage.this.getInputSlot());
                        InventoryUtil.dropItems(inventory, location, AdvancedStorage.this.getOutputSlot());
                    }
                }

                AdvancedStorage.this.locationMap.remove(location);
            }
        };
    }
